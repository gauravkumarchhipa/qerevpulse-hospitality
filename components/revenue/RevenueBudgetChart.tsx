// "use client";

// import {
//   BarChart as RechartsBarChart,
//   Bar,
//   ResponsiveContainer,
//   XAxis,
//   YAxis,
//   CartesianGrid,
//   Tooltip,
//   Legend,
//   LabelList,
// } from "recharts";
// import { useTheme } from "@/app/theme-provider";
// import { useState } from "react";
// import { formatCompact } from "./RevenueController";

// interface BarChartData {
//   name: string;
//   [key: string]: string | number;
// }

// interface DataKeyConfig {
//   key: string;
//   name: string;
//   color: string;
// }

// interface BarChartProps {
//   data: BarChartData[];
//   title: string;
//   height?: number;
//   showGrid?: boolean;
//   showLegend?: boolean;
//   yAxisFormatter?: (value: number) => string;
//   dataKeys?: DataKeyConfig[];
// }

// export default function RevenueBudgetChart({
//   data,
//   title,
//   height = 300,
//   showGrid = false,
//   showLegend = true,
//   yAxisFormatter = (v) => v.toString(),
//   dataKeys = [],
// }: BarChartProps) {
//   const { theme } = useTheme();
//   const isDark = theme === "dark";
//   const [hoveredLegend, setHoveredLegend] = useState<string | null>(null);

//   const renderWrappedTick = ({
//     x,
//     y,
//     payload,
//   }: {
//     x: number;
//     y: number;
//     payload: { value: string };
//   }) => {
//     const words = payload.value.split(" ");
//     return (
//       <text x={x} y={y} textAnchor="middle" fontSize={12} fill="#666">
//         {words.map((word, index) => (
//           <tspan key={index} x={x} dy={index === 0 ? "0.71em" : "1.2em"}>
//             {word}
//           </tspan>
//         ))}
//       </text>
//     );
//   };

//   return (
//     <div className="w-full h-full">
//       <div className="flex justify-center items-center mb-4">
//         <h3 className="text-base font-medium text-center">{title}</h3>
//       </div>

//       <ResponsiveContainer width="100%" height={height}>
//         <RechartsBarChart
//           data={data}
//           margin={{ top: 5, right: 30, left: 10, bottom: 50 }}
//         >
//           {showGrid && (
//             <CartesianGrid
//               strokeDasharray="3 3"
//               vertical={false}
//               stroke={isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}
//             />
//           )}
//           <XAxis
//             dataKey="name"
//             tick={renderWrappedTick}
//             tickLine={false}
//             dy={10}
//             axisLine={{
//               stroke: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)",
//             }}
//           />
//           <YAxis
//             // tickFormatter={yAxisFormatter}
//             tickFormatter={(v) => (v / 1_000_000).toFixed(2) + "M"}
//             tick={{ fontSize: 12 }}
//             tickLine={false}
//             axisLine={{
//               stroke: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)",
//             }}
//           />
//           <Tooltip
//             formatter={(value: number) => [`${formatCompact(value)}`, ""]}
//             contentStyle={{
//               backgroundColor: isDark ? "hsl(var(--card))" : "white",
//               borderColor: isDark ? "hsl(var(--border))" : "#e2e8f0",
//               borderRadius: "0.375rem",
//               fontSize: "0.875rem",
//             }}
//           />
//           {showLegend && (
//             <Legend
//               verticalAlign="top"
//               height={36}
//               iconSize={10}
//               iconType="rect"
//               wrapperStyle={{ fontSize: "0.75rem", cursor: "pointer" }}
//               onMouseEnter={(o) => setHoveredLegend(o.value)}
//               onMouseLeave={() => setHoveredLegend(null)}
//             />
//           )}
//           {dataKeys.map((d) => (
//             <Bar
//               key={d.key}
//               dataKey={d.key}
//               name={d.name}
//               fill={d.color}
//               radius={[4, 4, 0, 0]}
//               fillOpacity={
//                 hoveredLegend === null || hoveredLegend === d.name ? 1 : 0.3
//               }
//             >
//               <LabelList
//                 dataKey={d.key}
//                 position="top"
//                 formatter={(val: number) => `${formatCompact(val)}`}
//                 style={{
//                   fill: isDark ? "#fff" : "#000",
//                   fontSize: 12,
//                 }}
//               />
//             </Bar>
//           ))}
//         </RechartsBarChart>
//       </ResponsiveContainer>
//     </div>
//   );
// }







import {
  BarChart as RechartsBarChart,
  Bar,
  ResponsiveContainer,
  XAxis,
  YAxis,
  CartesianGrid,
  Tooltip,
  Legend,
  LabelList,
} from "recharts";
import { useTheme } from "@/app/theme-provider";
import { useState } from "react";
import { formatCompact } from "./RevenueController";

interface BarChartData {
  name: string;
  [key: string]: string | number;
}

interface DataKeyConfig {
  key: string;
  name: string;
  color: string;
}

interface BarChartProps {
  data: BarChartData[];
  title: string;
  height?: number;
  showGrid?: boolean;
  showLegend?: boolean;
  yAxisFormatter?: (value: number) => string;
  dataKeys?: DataKeyConfig[];
}

export default function RevenueBudgetChart({
  data,
  title,
  height = 300,
  showGrid = false,
  showLegend = true,
  yAxisFormatter = (v) => v.toString(),
  dataKeys = [],
}: BarChartProps) {
  const { theme } = useTheme();
  const isDark = theme === "dark";
  const [hoveredLegend, setHoveredLegend] = useState<string | null>(null);

  // Helper function for truncating labels with ellipsis if they exceed 12 characters
  const truncateLabel = (label: string, maxLength = 12) => {
    return label.length > maxLength ? `${label.slice(0, maxLength)}...` : label;
  };

  // Render function for the X-axis tick
  const renderWrappedTick = ({
    x,
    y,
    payload,
  }: {
    x: number;
    y: number;
    payload: { value: string };
  }) => {
    const truncatedLabel = truncateLabel(payload.value);
    return (
      <text x={x} y={y} textAnchor="middle" fontSize={12} fill="#666">
        <tspan>{truncatedLabel}</tspan>
      </text>
    );
  };

  return (
    <div className="w-full h-full">
      <div className="flex justify-center items-center mb-4">
        <h3 className="text-base font-medium text-center">{title}</h3>
      </div>

      <ResponsiveContainer width="100%" height={height}>
        <RechartsBarChart
          data={data}
          margin={{ top: 5, right: 30, left: 10, bottom: 50 }}
        >
          {showGrid && (
            <CartesianGrid
              strokeDasharray="3 3"
              vertical={false}
              stroke={isDark ? "rgba(255,255,255,0.1)" : "rgba(0,0,0,0.1)"}
            />
          )}
          <XAxis
            dataKey="name"
            tick={renderWrappedTick}
            tickLine={false}
            dy={10}
            axisLine={{
              stroke: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)",
            }}
            tickFormatter={(value) => {
              return truncateLabel(value, 12); // Truncate labels if needed
            }}
            interval={0} // Ensure every label is shown
            angle={-45} // Rotate labels if needed
          />
          <YAxis
            tickFormatter={yAxisFormatter}
            tick={{ fontSize: 12 }}
            tickLine={false}
            axisLine={{
              stroke: isDark ? "rgba(255,255,255,0.2)" : "rgba(0,0,0,0.1)",
            }}
          />
          <Tooltip
            formatter={(value: number) => [`${formatCompact(value)}`, ""]}
            contentStyle={{
              backgroundColor: isDark ? "hsl(var(--card))" : "white",
              borderColor: isDark ? "hsl(var(--border))" : "#e2e8f0",
              borderRadius: "0.375rem",
              fontSize: "0.875rem",
            }}
          />
          {showLegend && (
            <Legend
              verticalAlign="top"
              height={36}
              iconSize={10}
              iconType="rect"
              wrapperStyle={{ fontSize: "0.75rem", cursor: "pointer" }}
              onMouseEnter={(o) => setHoveredLegend(o.value)}
              onMouseLeave={() => setHoveredLegend(null)}
            />
          )}
          {dataKeys.map((d) => (
            <Bar
              key={d.key}
              dataKey={d.key}
              name={d.name}
              fill={d.color}
              radius={[4, 4, 0, 0]}
              fillOpacity={
                hoveredLegend === null || hoveredLegend === d.name ? 1 : 0.3
              }
            >
              <LabelList
                dataKey={d.key}
                position="top"
                formatter={(val: number) => `${formatCompact(val)}`}
                style={{
                  fill: isDark ? "#fff" : "#000",
                  fontSize: 12,
                }}
              />
            </Bar>
          ))}
        </RechartsBarChart>
      </ResponsiveContainer>
    </div>
  );
}
